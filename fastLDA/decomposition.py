...
from sklearn.utils import (check_random_state, check_array, gen_batches, gen_even_slices)
from sklearn.utils.validation import check_non_negative
from sklearn.utils.fixes import logsumexp
from sklearn.utils.validation import check_non_negative
from sklearn.utils.validation import check_is_fitted

from sklearn.decomposition._online_lda_fast import (mean_change, _dirichlet_expectation_1d,
...
class LatentDirichletAllocation_fast(TransformerMixin, BaseEstimator):
    ...
    def __init__(self, n_components=10, doc_topic_prior=None,
        ...
        self.learning_method = learning_method ## Only 'batch'
    ...
    def _e_step(self, X, cal_sstats, random_init, parallel=None):
        ...
        if parallel is None:
            parallel = Parallel(n_jobs=n_jobs, verbose=max(0,
                                                           self.verbose - 1), max_nbytes=None)
        results = parallel(
            delayed(_update_doc_distribution_cython)(X[idx_slice, :],
    ...
    def fit(self, X, y=None):
        ...
        with Parallel(n_jobs=n_jobs,
                      verbose=max(0, self.verbose - 1), max_nbytes=None) as parallel:
...
#                                                      sub_sampling=False)
        ...
        doc_topics_distr /= doc_topics_distr.sum(axis=1)[:, np.newaxis]
        return doc_topics_distr



    # All the code below is useless
